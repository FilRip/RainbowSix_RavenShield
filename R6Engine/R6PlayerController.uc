...
simulated function ResetOriginalData ()
{
	...
	m_GameOptions=Class'Actor'.UnknownFunction1009();
	...
	if ( (Level.NetMode == 3) || (Level.NetMode == 2) && (Viewport(Player) != None) )
	{
		ServerSetGender(m_GameOptions.Gender > 0);
	}
	ResetBlur();
	if ( (m_CurrentCircumstantialAction == None) && (Role == 4) )
	{
...
event InitMultiPlayerOptions ()
{
	...
	ToggleRadar(UnknownFunction1009().ShowRadar);
...
{
	if ( (Level.NetMode == 1) || (m_GameService == None) )
	{
		return;
	}
...
{
	if ( (Level.NetMode == 1) || (m_GameService == None) || (PlayerReplicationInfo.m_bClientWillSubmitResult == False) )
	{
		return;
	}
...
function ClientNotifySendMatchResults ()
{
	...
	m_GameService.UnknownFunction1297();
}
...
simulated event PostBeginPlay ()
{
	...
	if ( Role == 4 )
	{
		...
		if ( (Level.NetMode == 0) || Level.IsGameTypeCooperative(Level.Game.m_szGameTypeFlag) )
		{
	...
	UnknownFunction2011(False);
	m_GameOptions=Class'Actor'.UnknownFunction1009();
}

function UpdateTriggerLagInfo ()
{
	if ( (m_GameOptions != None) && ((Level.NetMode == 3) || (Pawn != None) && Pawn.IsLocallyControlled()) )
	{
...
simulated function HidePlanningActors ()
{
	...
	{
		if (  !ExtZone.UnknownFunction1513(szCurrentGameType) )
		{
...
event PostRender (Canvas Canvas)
{
	...
	if ( Pawn != None )
	{
		...
		Canvas.UnknownFunction2005(iBlurValue);
	} else {
		Canvas.UnknownFunction2005(0);
	}
	if (  !m_bBombSearched )
	{
		...
		if ( Level.NetMode == 0 )
		{
			if ( (m_pSelfDetonatingBomb != None) && (Level.NetMode != 3) )
			{
				foreach AllActors(Class'R6IOSelfDetonatingBomb',AIt)
				{
					m_pSelfDetonatingBomb=AIt;
...
simulated function RenderTimeLeft (Canvas C)
{
	...
	sTime=sTime $ UnknownFunction1520(iTimeLeft,True);
	C.UnknownFunction1606(True,640.00,480.00);
	X=C.HalfClipX;
	Y=C.HalfClipY / 8;
	C.Font=Font'Rainbow6_14pt';
	if ( iTimeLeft > 20 )
	{
		C.UnknownFunction2626(255,255,255);
	} else {
		if ( iTimeLeft > 10 )
		{
			C.UnknownFunction2626(255,255,0);
		} else {
			C.UnknownFunction2626(255,0,0);
		}
	}
	C.StrLen(sTime,fStrSizeX,fStrSizeY);
	C.UnknownFunction2623(X - fStrSizeX / 2,Y + 24);
	C.DrawText(sTime);
}
...
{
	local PatchState PatchState;

	if ( m_bQuitToUpdateServerDisplayed == False )
	{
		if ( Level.TimeSeconds - m_fLastUpdateServerCheckTime > 5 )
		{
			m_fLastUpdateServerCheckTime=Level.TimeSeconds;
			PatchState=Class'R6AbstractEviLPatchService'.GetState();
			if ( PatchState == 6 )
			{
	...
	{
		UnknownFunction2211();
		UnknownFunction1843(fDeltaTime);
...
JL01AC:
	...
	{
		rViewRotation.Roll=m_pawn.UnknownFunction1508(m_pawn.m_fPeeking) * 2049;
	}
	...
	if ( ((Pawn.Acceleration != vect(0.00,0.00,0.00)) || (aForward != 0) || (aStrafe != 0)) &&  !m_pawn.m_bIsClimbingLadder )
	{
		...
		if (  !bBoneRotationIsDone && m_pawn.m_bMovingDiagonally &&  !m_pawn.m_bIsProne )
		{
			...
			m_pawn.UnknownFunction2214(rRotationOffset,True,True);
			rRotationOffset.Yaw=0;
		...
		if ( m_pawn.m_bIsProne )
		{
			...
			if ( PlayerIsFiring() )
			{
				m_pawn.UnknownFunction2214(rRotationOffset,True,False);
				bBoneRotationIsDone=True;
			}
		} else {
			if ( (aForward == 0) && (aStrafe == 0) && m_pawn.m_bMovingDiagonally )
			{
				...
				if ( PassedYawLimit(rRotationOffset) || (rRotationOffset.Yaw != 0) && m_pawn.IsPeeking() )
				{
					...
					bRotateToDesired=True;
					rRotationOffset.Yaw=0;
					m_pawn.UnknownFunction2214(rRotationOffset,);
					bBoneRotationIsDone=True;
				}
			}
		}
	...
	{
		m_pawn.UnknownFunction2214(rRotationOffset,,True);
	}
	ViewFlash(DeltaTime);
...
function ResetFluidPeeking ()
{
	...
	{
		SetPeekingInfo(0,m_pawn.1000.00);
		SetCrouchBlend(0.00);
	}
}
...
function HandleFluidMovement (float DeltaTime)
{
	...
	if ( (m_bSpecialCrouch > 0) &&  !m_pawn.m_bIsProne )
	{
		...
		fPeekingRate=m_pawn.UnknownFunction1508(m_pawn.m_fPeeking);
...
exec function RaisePosture ()
{
	...
	if ( m_pawn.m_ePeekingMode == 2 )
	{
		if (  !m_pawn.UnknownFunction2200(0.00,True) )
		{
			return;
		}
		m_pawn.UnknownFunction2200(0.00);
		ResetFluidPeeking();
	}
	if ( m_bCrawl )
	{
...
exec function LowerPosture ()
{
	...
	if ( m_pawn.m_ePeekingMode == 2 )
	{
		if ( bDuck == 0 )
		{
			m_pawn.UnknownFunction2200(0.96);
		}
		ResetFluidPeeking();
	}
	if ( bDuck == 0 )
...
exec function ToggleAutoAim ()
{
	if ( Level.NetMode == 0 )
	{
		...
		Class'Actor'.UnknownFunction1009().AutoTargetSlider=m_wAutoAim;
	} else {
		m_wAutoAim=0;
	}
}
	...
	if ( m_pawn.m_ePeekingMode == 1 )
	{
		if ( m_pawn.IsPeekingLeft() )
		{
			if ( bPeekLeftButton == 0 )
			{
				...
				{
					SetPeekingInfo(1,m_pawn.2000.00);
				} else {
					SetPeekingInfo(0,m_pawn.1000.00);
				}
			}
			...
			if ( bPeekRightButton == 0 )
			{
				...
				{
					SetPeekingInfo(1,m_pawn.0.00,True);
				} else {
					SetPeekingInfo(0,m_pawn.1000.00);
				}
			}
		...
		if (  !(m_pawn.m_ePeekingMode == 1) && m_pawn.CanPeek() )
		{
			if ( bPeekLeftButton > 0 )
			{
				...
				{
					ResetSpecialCrouch();
					SetPeekingInfo(1,m_pawn.2000.00,False);
				}
			}
		}
	}
...
state PlayerFlying extends PlayerFlying
{
	function BeginState ()
	{
		if ( Pawn != None )
		{
			...
			m_pawn.UnknownFunction2214(Pawn.m_rRotationOffset,,True);
			Pawn.SetPhysics(4);
		}
	}
...
state GameEnded extends GameEnded
{
}
...
auto state BaseSpectating extends BaseSpectating
{
...
state PauseController extends PlayerWalking
{
	...
	{
		bFire=0;
		Super.ProcessMove(DeltaTime,vect(0.00,0.00,0.00),0,DeltaRot);
	}
...
JL0043:
	...
	iMaxPlayerOnTeam=UnknownFunction1302().GetMaxNbPlayers(GameReplicationInfo.m_szGameTypeFlagRep);
	...
	if (  !bSameTeam )
	{
		...
		if ( (eTeamSelected == 2) && (iTeamA >= iMaxPlayerOnTeam) || (eTeamSelected == 3) && (iTeamB >= iMaxPlayerOnTeam) )
		{
	...
	if (  !bSameTeam )
	{
		...
		foreach UnknownFunction313(Class'R6PlayerController',P)
		{
...
state Dead extends Dead
{
	...
	simulated function BeginState ()
	{
		...
		if ( (Level.NetMode == 1) || (Level.NetMode == 2) && (Viewport(Player) == None) )
		{
		...
		if ( (Level.NetMode == 3) || (Level.NetMode == 2) && (Viewport(Player) != None) )
		{
			if ( (m_MenuCommunication != None) && (m_TeamSelection != 4) && ( !GameReplicationInfo.IsInAGameState() || (Pawn != None)) )
			{
				...
				if ( (Level.NetMode == 2) && (Viewport(Player) != None) && (m_TeamSelection != 4) )
				{
					if ( m_MenuCommunication == None )
					{
						InitializeMenuCom();
...
function DisableFirstPersonViewEffects (optional bool bChangingPawn)
{
	...
	if ( Pawn != None )
	{
		if ( Pawn.IsLocallyControlled() )
		{
			...
			R6Pawn(Pawn).UnknownFunction2004(False,None,None);
			R6Pawn(Pawn).UnknownFunction2600(False,None,None);
			R6Pawn(Pawn).UnknownFunction2605(False,None,None);
			Level.m_bHeartBeatOn=False;
			ResetBlur();
			Level.m_bInGamePlanningActive=False;
			UnknownFunction2011(False);
			if ( (Level.NetMode == 0) ||  !PlayerCanSwitchToAIBackup() )
			{
				AWeapon=R6AbstractWeapon(Pawn.EngineWeapon);
				if ( AWeapon != None )
				{
...
state CameraPlayer
{
	...
	simulated function SetSpectatorRotation ()
	{
		...
		if ( ViewTarget != None )
		{
			if (  !bBehindView )
			{
				SetRotation(ViewTarget.Rotation + R6Pawn(ViewTarget).UnknownFunction1842());
			} else {
	...
	simulated function PlayerMove (float DeltaTime)
	{
		...
		if ( Class'Actor'.UnknownFunction1524().IsMissionPack() )
		{
	...
	simulated function Tick (float fDeltaTime)
	{
		...
		if ( m_bSpectatorCameraTeamOnly && ((m_iTeamId == 2) || (m_iTeamId == 3)) && (ViewTarget != None) && (Pawn(ViewTarget).m_iTeam != m_iTeamId) )
		{
			if ( Level.NetMode != 3 )
			{
				SpectatorChangeTeams(True);
	...
	simulated function SetNewViewTarget (Actor aViewTarget)
	{
		...
		if ( aPawn.Controller != None )
		{
			...
			if ( (Role == 4) && (aOldTeamManager != None) && (aOldTeamManager != m_TeamManager) &&  !aOldTeamManager.m_bLeaderIsAPlayer &&  !m_TeamManager.m_bLeaderIsAPlayer )
			{
		...
		SetSpectatorRotation();
		FixFOV();
		if ( (Level.NetMode == 2) && (Viewport(Player) != None) )
		{
			DisplayClientMessage();
		}
	}
	...
	function DisplayClientMessage ()
	{
		...
		if ( (Level.NetMode == 3) || (Level.NetMode == 0) || (Level.NetMode == 2) && (Viewport(Player) != None) )
		{
			if ( bCheatFlying )
			{
...
state PlayerStartSurrenderSequence extends PlayerWalking
{
	function BeginState ()
	{
		...
		if ( m_pawn.m_bIsClimbingLadder )
		{
			...
			Pawn.SetLocation(Pawn.Location + 25 * Pawn.Rotation);
			m_pawn.PlayFalling();
		} else {
			if ( m_bCrawl )
			{
	...
	{
		if ( (Role == 4) && (Pawn.Physics != 2) &&  !Pawn.m_bIsLanding &&  !m_bCrawl && (bDuck == 0) )
		{
			GotoState('PlayerPreBeginSurrending');
...
state PlayerStartSurrending extends CameraPlayer
{
	...
	event AnimEnd (int iChannel)
	{
		if ( iChannel == m_pawn.16 )
		{
			...
			if ( Level.TimeSeconds - m_fStartSurrenderTime > 2 )
			{
				m_pawn.EngineWeapon.ServerStopFire();
				GotoState('PlayerSurrended');
			}
...
state PlayerSurrended extends CameraPlayer
{
	...
	event Tick (float fDeltaTime)
	{
		if ( (Role == 4) && (Level.TimeSeconds - m_fStartSurrenderTime > 10) &&  !m_pawn.m_bIsUnderArrest &&  !m_pawn.m_bIsBeingArrestedOrFreed )
		{
			m_bSkipBeginState=False;
			m_pawn.m_eHealth=0;
...
function ServerPlayerActionProgress ()
{
	...
	if ( m_PlayerCurrentCA.aQueryTarget.IsA('R6Terrorist') )
	{
		...
		if ( Class'Actor'.UnknownFunction1524().IsMissionPack() && m_PlayerCurrentCA.aQueryTarget.IsA('R6Rainbow') )
		{
...
state PlayerActionProgress extends PlayerWalking
{
	function BeginState ()
	{
		...
		if ( (Level.NetMode != 0) && (Role == 4) && m_PlayerCurrentCA.aQueryTarget.IsA('R6IOBomb') )
		{
	...
	function EndState ()
	{
		...
		if ( m_pawn != None )
		{
			...
			if ( (Role == 4) &&  !m_pawn.IsAlive() && (m_iPlayerCAProgress < 105) )
			{
				m_RequestedCircumstantialAction.aQueryTarget.R6CircumstantialActionCancel();
			}
		}
...
state PlayerClimbing extends PlayerClimbing
{
		...
		if ( (iChannel == 0) || (iChannel == m_pawn.1) )
		{
			if ( iChannel == 0 )
			{
				if ( m_pawn.m_Ladder.m_bIsTopOfLadder )
				{
					...
					Pawn.SetLocation(Pawn.Location + 20 * Pawn.Rotation);
				} else {
...
simulated function ClientFadeSound (float fTime, int iVolume, ESoundSlot eSlot)
{
	if ( Viewport(Player) != None )
	{
		UnknownFunction2721(fTime,iVolume,eSlot);
	}
}
...
simulated function ClientFadeCommonSound (float fTime, int iVolume)
{
	if ( Viewport(Player) != None )
	{
		UnknownFunction2721(fTime,iVolume,1);
		UnknownFunction2721(fTime,iVolume,2);
		UnknownFunction2721(fTime,iVolume,3);
		UnknownFunction2721(fTime,iVolume,4);
		UnknownFunction2721(fTime,iVolume,6);
		UnknownFunction2721(fTime,iVolume,8);
		UnknownFunction2721(fTime,iVolume,10);
		UnknownFunction2721(fTime,iVolume,11);
	}
}
...
event ClientPlayVoices (R6SoundReplicationInfo aAudioRepInfo, Sound sndPlayVoice, ESoundSlot eSlotUse, int iPriority, optional bool bWaitToFinishSound, optional float fTime)
{
	...
	{
		aAudioRepInfo.m_pawnOwner.UnknownFunction2731();
		aAudioRepInfo.m_pawnOwner.m_fLastCommunicationTime=5.00;
	}
	UnknownFunction2726(aAudioRepInfo,sndPlayVoice,eSlotUse,iPriority,bWaitToFinishSound,fTime);
}
...
JL008B:
	if ( (iIterator < _GRI.32) && (_GRI.m_mapArray[iIterator] != "") )
	{
		...
		Class'Actor'.UnknownFunction2620(szMapId $ ": " $ string(iIterator + 1) $ " " $ szMapName $ ": " $ _GRI.m_mapArray[iIterator] $ " " $ szLocGameType $ ": " $ _GRI.Level.GetGameNameLocalization(szGameType),myHUD.m_ServerMessagesColor);
		iIterator++;
		goto JL008B;
	}
}
...
exec function Map (int iGotoMapId, string explanation)
{
	...
	{
		Class'Actor'.UnknownFunction2620(Localize("Game","BadMapId","R6GameInfo"),myHUD.m_ServerMessagesColor);
		return;
	}
	Class'Actor'.UnknownFunction2620(Localize("Game","RequestingMap","R6GameInfo") $ ": " $ _GRI.m_mapArray[iGotoMapId],myHUD.m_ServerMessagesColor);
...
exec function PlayerList ()
{
	...
	{
		Class'Actor'.UnknownFunction2620(szID $ ": " $ string(_PRI.PlayerID) $ " " $ szName $ ": " $ _PRI.PlayerName,myHUD.m_ServerMessagesColor);
	}
}

exec function VoteKick (string szKickName)
{
	ProcessVoteKickRequest(R6PlayerController(UnknownFunction1224(szKickName,False)));
}

exec function VoteKickID (string szKickName)
{
	ProcessVoteKickRequest(R6PlayerController(UnknownFunction1224(szKickName,True)));
}
...
exec function Vote (int _bVoteResult)
{
	...
	pServerInfo=Class'Actor'.UnknownFunction1273();
...
function ClientPasswordMessage (eGamePasswordRes iMessageType)
{
	...
	{
		case 1:
		UnknownFunction2620(Localize("Game","GamePasswordMissing","R6GameInfo"),myHUD.m_ServerMessagesColor);
		break;
		case 2:
		UnknownFunction2620(Localize("Game","GamePasswordSet","R6GameInfo"),myHUD.m_ServerMessagesColor);
		break;
		case 3:
		UnknownFunction2620(Localize("Game","GamePasswordCleared","R6GameInfo"),myHUD.m_ServerMessagesColor);
		break;
		default:
	}
}
...
function bool CheckAuthority (int _LevelNeeded)
{
	...
	return (m_iAdmin >= _LevelNeeded) || (Level.NetMode == 2) && (Viewport(Player) != None);
}

exec function Kick (string szKickName)
{
	ProcessKickRequest(R6PlayerController(UnknownFunction1224(szKickName,False)));
}

exec function KickId (string szKickName)
{
	ProcessKickRequest(R6PlayerController(UnknownFunction1224(szKickName,True)));
}
...
exec function Ban (string szKickName)
{
	...
	PC=R6PlayerController(UnknownFunction1224(szKickName,False));
...
exec function BanId (string szKickName)
{
	...
	PC=R6PlayerController(UnknownFunction1224(szKickName,True));
	ProcessKickRequest(PC,True);
}
...
function ClientNoBanMatches ()
{
	...
	UnknownFunction2620(Localize("Game","NoBanMatchFound","R6GameInfo"),myHUD.m_ServerMessagesColor);
	iPos=0;
JL0041:
	if ( iPos < 10 )
	{
...
{
	UnknownFunction2620(Localize("Game","PlayerUnBanned","R6GameInfo"),myHUD.m_ServerMessagesColor);
}

function ClientPBVersionMismatch ()
{
	UnknownFunction2620(Localize("Game","PBVersionMismatch","R6GameInfo"),myHUD.m_ServerMessagesColor);
}
...
JL001D:
	if ( iPos < 10 )
	{
		...
		UnknownFunction2620(string(iPos) $ "> " $ m_BanPage.szBanID[iPos],myHUD.m_ServerMessagesColor);
		iPos++;
		goto JL001D;
	}
JL007D:
...
exec function UnBanPos (int iPosition)
{
	...
	{
		UnknownFunction2620(Localize("Game","NoBannedInPos","R6GameInfo"),myHUD.m_ServerMessagesColor);
		return;
	}
...
exec function NextBanList ()
{
	...
	if ( m_iBanPage == 0 )
	{
		UnknownFunction2620(Localize("Game","BanListFirst","R6GameInfo"),myHUD.m_ServerMessagesColor);
	} else {
...
simulated function ProcessKickRequest (R6PlayerController _playerController, optional bool bBan)
{
	...
	_playerController.UnknownFunction1282();
}
...
function SendSettingsAndRestartServer (bool _bRestrictionKitChange, bool _bChangeWasMade)
{
	...
	if ( _bChangeWasMade )
	{
		pServerInfo.SaveConfig(Class'Actor'.UnknownFunction1524().GetServerIni());
		pServerInfo.m_ServerMapList.SaveConfig(Class'Actor'.UnknownFunction1524().GetServerIni());
...
{
	local Class<R6Rainbow> TempGreenClass;
	local Class<R6Rainbow> TempRedClass;
...
defaultproperties
{
    ...
    m_stImpactHit=(iBlurIntensity=170002799, fWaveTime=3.86856262276681344E25, fRollMax=0.00, fRollSpeed=0.00, fReturnTime=0.00)
    m_stImpactStun=(iBlurIntensity=337774959, fWaveTime=3.86856262276681344E25, fRollMax=0.00, fRollSpeed=0.00, fReturnTime=0.00)
    m_stImpactDazed=(iBlurIntensity=673319279, fWaveTime=3.86856262276681344E25, fRollMax=0.00, fRollSpeed=0.00, fReturnTime=-6.77000119191158912E25)
    m_stImpactKO=(iBlurIntensity=1260521839, fWaveTime=3.96140812571321664E28, fRollMax=0.00, fRollSpeed=0.00, fReturnTime=0.00)
